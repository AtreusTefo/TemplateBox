...
* Passive banners only, selected by viewport band: a three-slot 300x250 rail beside the workspace at 93rem and above, a single 160x600 rail between 75rem and 93rem, a 728x90 leaderboard above the workspace between 48rem and 75rem, or a 320x50 anchor fixed to the foot of the viewport below 48rem. The bands are mutually exclusive to the pixel: never two bands at once, never none.
* Each slot in the rail stack must carry its own Adsterra zone key. Repeating one key down the stack is not a valid substitute.
* The rail gate is 75rem, matching the homepage (lowered from 84rem on August 13, 2026). 84rem was the point below which the page is viewport-capped and a rail comes out of the editing panes rather than out of spare margin; the lower gate accepts that cost deliberately, for parity across page types on common laptop widths. See both pane-width tables in the implementation note.
