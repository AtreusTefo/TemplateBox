...
- **No Pop-Under, as of August 6, 2026.** It was removed because it redirected visitors to third-party sites on ordinary clicks. The ~150-line click shield and overlay neutralizer in `site/index.html` that existed solely to contain it went with it, as did the deferred-navigation/watchdog machinery in `launchTemplate()`. `index.html` now carries zero advertising of any kind. Do not reintroduce a Pop-Under, In-Page Push, or any other format that can navigate the visitor's tab without an explicit click on an ad.
- **Ad policy (revised July 27, 2026):** `index.html` carries no banner, ever. Editors carry passive banners only, selected by viewport band: a three-slot 300x250 rail at 93rem and above, a single 160x600 rail between 84rem and 93rem, a 728x90 leaderboard between 48rem and 84rem, a 320x50 anchor below 48rem. Every slot in the stack carries its own zone key. No active format, and no placement inside the form or preview panes. Every placement is declared once in `AD_ZONES` in `site/js/ads.js` and rendered iframe-isolated; page-context formats such as Native Banner are prohibited on editors because they would run third-party code in the same document as the user's typed content. See `docs/implementation/EDITOR_PAGE_AD_PLACEMENT.md`.
