...
- **Tablets carry the phone anchor now (August 20, 2026).** Reported as "there are no ads on the tablet screens", and it was true of every page family except the editors: the anchor's ceiling was 48rem, the rails' floor is 75rem, and 769-1199px -- every tablet in portrait, most in landscape -- got nothing. `SITE_ANCHOR_MAX` and `HOME_ANCHOR_MAX` end at **74.9375rem** now, one pixel below the rail floor, so the two bands meet exactly and no viewport mounts both or neither. **The "and nothing between" in the next bullet no longer describes the site** -- it is kept there as the history of the rail's own gate, not as current behaviour. Three things this touched that are easy to get wrong again: `loading.html` hides its second 300x250 below **48rem** now rather than 70rem (the pair fit side by side from 769px, so the stacking argument that justified cutting it does not apply on a tablet, and the page keeps zero vertical scroll -- verified at seven tablet sizes); `body.has-site-anchor` reserves **one** value, 4.75rem, because `mountSiteAnchor` only ever mounts a 320x50 -- the old 7.25rem tier was sized for a 728x90 retired on August 13 and silently over-reserved 116px under a 50px bar the moment the gate moved; and `RAIL_GAP` in `tests/verify-layout.js` had been asserting the hole was correct, so **check that table before trusting a band-count failure**. The editors are unchanged: they already had a 728x90 in that band, and swapping it for the phone anchor would break the four-band invariant below and cost ad quality on the site's longest sessions.
...
- **Non-Editor Ad Coverage:** The same "never two bands and never none" rule holds on every other page family too, and since August 20, 2026 there is no width at which one of them shows nothing: the anchor runs to 74.9375rem and the rail starts at 75rem. `404.html` above 75rem is the one deliberate exception (it was left out of the content-rail rollout). Verify by loading each width fresh -- **`js/ads.js` reads `matchMedia` once at mount and never remounts, so resizing the window proves nothing about which band a visitor would actually get.** Also verify that whatever the anchor reserves in body padding matches the height of the unit that actually mounted, in both directions: under-reserving strands the foot of the document beneath a fixed bar, over-reserving leaves dead space, and both are silent.
