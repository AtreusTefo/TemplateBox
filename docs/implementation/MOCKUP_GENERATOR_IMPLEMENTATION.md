...
- `css/style.css` — mockup catalog card preview variants (`.mock-doc.mockup` + `.dot-row`/`.dot` color swatches), and the editor's own styles: `.mockup-stage`, `.color-row` (the Background panel's quick picks; the Colour panel's row became `.color-hue-inline` on August 25, 2026), `.range-row`, `.mockup-tray` and its `.tray-*` grid/card/thumbnail rules.
