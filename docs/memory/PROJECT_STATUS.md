...
A 100% client-side, serverless template personalization engine at templatebox.win. Two live tools today: a CV/Resume builder and a Poster/Canvas creator. Monetized through a mandatory 10-second ad loading page between the catalog and the editor. Full requirements and design system live in `PRD.md` and `CLAUDE.md`.
...
```
index.html          Catalog: 6 template cards (3 resume, 3 poster), category filter pills, Popunder script, JSON-LD WebApplication schema
...
Pattern demonstrated by resume.html/poster.html, to follow for a third tool:
