...
Pattern demonstrated by resume.html/poster.html/mockup.html, to follow for a fourth tool:

1. Add a card to the `.catalog-grid` in `index.html` with a `data-target="yourkey"` button and a `data-category` matching an existing or new filter pill.
2. Add `yourkey: "yourpage.html"` to `EDITOR_ROUTES` in `js/app.js` — this is what keeps the loading-page redirect safe from tampering.
